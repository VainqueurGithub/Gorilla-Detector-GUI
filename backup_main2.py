...
def start_task(conf, iou):
    ...
    popup_progress_chunk_spect()
 
    # Run the task in a separate thread to avoid blocking the main GUI
    threading.Thread(target=lambda: (time.sleep(3), popup_progress_detect(conf,iou)), daemon=True).start()

def popup_progress_chunk_spect():
    # Create a new popup window
    global popup, progress, label
    popup = tk.Toplevel(root)
    popup.title("Spectrogram generation")
    ...
    threading.Thread(target=task_progress_bar_chunk_spectrogram, args=(progress, label, popup), daemon=True).start()

def popup_progress_detect(conf,iou):
    # Create a new popup window
    global popup, progress, label
    popup = tk.Toplevel(root)
...
def task_progress_bar_chunk_spectrogram(progress, label, popup):
    ...
    for i, file in enumerate(files, start=1):
        ...
        chunk_spectrogram(filename,input_path,path_audio_chunks,directory_chunk)
    ...
    label.config(text="Spectrogram generation Completed!")
    popup.after(1000, popup.destroy)
    run_button.config(state="normal")

def task_progress_bar_detection(progress, label, popup, conf,iou): 
    ...
    spects = [os.path.join(path_audio_chunk_spect, p) for p in os.listdir(path_audio_chunk_spect) if p.endswith(".PNG") or p.endswith(".png")]
    progress["value"] = 0
    progress["maximum"] = max_value

    for j, spect in enumerate(spects, start=1):
        spectname = os.path.basename(spect)
        detection_out_put(spectname,input_path,path_audio_chunks,conf, iou, output_path,path_audio_chunk_spect,detector_table,audio_duration,max_frequency,View,Channel,label)
        time.sleep(0.01 + (0.01 * ((j) / max_value)))
        progress["value"] = j
        label.config(text=f"Processing {j}/{len(spects)}")
        root.update_idletasks()  # Refresh the GUI
...
def detection_out_put(filename_chunk_spect,input_path,path_audio_chunks,conf, iou, output_path,path_audio_chunk_spect,detector_table,audio_duration,max_frequency,View,Channel,label):
    Selection=1
    View = 'Spectrogram 1'
    Channel = 1
    label='Gorilla'
...
def chunk_spectrogram(filename,input_path,path_audio_chunks,directory_chunk,hop_length=hop_length, n_mels=n_mels, time_steps=time_steps):
